/* NetId(s): yb244, xl597. Time spent: 8 hours, 30 minutes.

 * Name(s): Yihan Bao, Xinci Liu
 * What I thought about this assignment:
 * I think the assignment is a bit hard to understand at the beginning,
 * because the concepts are a little complicated, but things go smoothly then.
 *
 */
...
public class RepostTree {
    ...
    public int depth(Person p) {
    	...
    	for(RepostTree rt: children)
    		if (rt.depth(p)!= -1) return rt.depth(p)+1;
	return -1;
    ...
    public int widthAtDepth(int d) throws IllegalArgumentException {
    	...
    	int width = 0;
    for(RepostTree rt : children) {
    		width = width + rt.widthAtDepth(d-1);
    	}
    ...
    public List<Person> getRepostRoute(Person c) {
    	...
    	LinkedList<Person> route = new LinkedList<Person>();
    	if(root == c) {
       		route.addLast(c);
    		return route;
    	}
		for(RepostTree rt: children) {
			if(rt.contains(c)) {
				route = (LinkedList<Person>) rt.getRepostRoute(c);
		  		route.addFirst(root);
		  		return route;
			}
		}
    ...
    public boolean equals(Object ob) {
    	...
    	RepostTree newob = (RepostTree) ob;
    	if(newob == this) return true;   	
    	if(newob.getRoot() != getRoot()) return false;
    	if(newob.getChildrenCount() !=getChildrenCount()) return false;
    	Set<RepostTree> set1 = newob.getChildren();
    	Set<RepostTree> set2 = getChildren();
    	for(RepostTree tr2: set2) {
    	if(!childrenEquals(set1,tr2)) {
    		return false;
    	} 
    }
    	return true;
    ...
    /**
     * Return true iff for RepostTree tr, there is a RepostTree i in set1, 
     * for which tr.equals(i).
     * @param set1
     * @param tr
     * @return 
     */
    private boolean childrenEquals(Set<RepostTree> set1, RepostTree tr) {
        for(RepostTree i: set1) {
        	if(tr.equals(i))
                return true;
               }
        return false;
